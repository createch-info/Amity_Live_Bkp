            ...
            foreach ($d->schedules as $schedule)
            {
                ...
                switch ($schedule->schedules)
                {
                    case 1:
                        ...
                        $seminarDateformated= Carbon::parse($d->seminar_date)->format("m-d-Y");
                        $startTime= Carbon::parse($d->start_time)->format("g:i A");
                            ...
                            foreach ($d->registrants as $registrant)
                            {
                                Log::info('Registrants'.$registrant);
                                    if($registrant->choice_of_communication=='Email & Text')
                                    {
                                         ...
                                         $this->sendSMS(countryCodeId.$registrant->phoneNumber, "This is a reminder for seminar \"".$d->title."\" at $startTime on $seminarDateformated . Please check your email for details and call us with any questions at 303-690-2749.");
                                    }
                                    else
                                        Mail::to($registrant->email)->send(new ReminderEmail($d,'paypal',$registrant->accommodation));
                    ...
                    if($diff<=60 && $diff>0)
                    {
                        foreach ($d->registrants as $registrant)
                        {
                           ...
                           if($registrant->choice_of_communication=='Email & Text')
                            {
                                Mail::to($registrant->email)->send(new ReminderEmail($d,'paypal',$registrant->accommodation));
                                $this->sendSMS(countryCodeIdId.$registrant->phoneNumber, "This is a reminder for seminar \"".$d->title."\" at $startTime on $seminarDateformated . Please check your email for details and call us with any questions at 303-690-2749.");
                            }
                            else
                                Mail::to($registrant->email)->send(new ReminderEmail($d,'paypal',$registrant->accommodation));
                        }
